...
helper.loadTemplateWithClass = function(elementId, type, name){
  var path = '/pwa/templates/html-min/' + type + '/' + name + '/' + name + '.html';
  ...
  xhr.onreadystatechange = function() {
    if (xhr.readyState === 4 && xhr.status == 200) {
      ...
      var fileName = name + '/' + name + '-min.js';
      var revFileName = _Files && _Files["revFiles"][fileName];
      script.type = 'text/javascript';

      script.src = '/pwa/templates/js-min/' + revFileName;
      //Switch these for debugging.
      // script.src = '/pwa/templates/' +type+ '/' + name + '/' + name + '.js';
...
helper.loadPage = function (name, id) {
  return new Promise(function(resolve){
    var pageElm = document.getElementById(id);
    if (!pageElm) {
      return;
    }
    setTimeout(function () {
      helper.removeClass(pageElm, 'slide-out');
      helper.removeClass(pageElm, 'slide-in');
      helper.addClass(pageElm, 'slide-out');
    }, 0);

    var path = '/pwa/templates/html-min/pages/' + name + '/' + name + '.html';
    var xhr = typeof XMLHttpRequest !== 'undefined' ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    xhr.open('get', path, true);
    xhr.onreadystatechange = function () {
      if (xhr.readyState === 4 && xhr.status == 200) {

        pageElm.innerHTML = getRevCss(xhr.responseText, name);
        var scriptID = name + '-page-id';
        var script = document.getElementById(scriptID);
        if (script) {
          script.parentElement.removeChild(script);
        }
        var body = document.getElementsByTagName('head')[0];
        var fileName = name + '/' + name + '-concatenation-min.js';
        //Switch these for debugging.
        // var fileName = name + '/' + name + '-concatenation-debug.js';
        var revFileName = _Files && _Files["revFiles"][fileName];
        ...
        script.addEventListener('load', function() {
          helper.addClass(pageElm, 'slide-in');
          if(name !== '404') {
            _Router.dispatchAds();
          }
          resolve();
...
var iconIds = ['icon', 'day.icon', 'night.icon'];
